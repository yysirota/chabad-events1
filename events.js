...
if(base&&!css){css=d.createElement("link");css.id="cfle-events-css";css.rel="stylesheet";css.type="text/css";css.href=base+"events.css?v=4";head.appendChild(css);}

var CFG={feedUrl:"/templates/events.htm",cacheKey:"cfleEventsV4",cacheMs:600000},
...
function ensureRenderContainers(){
  var mount=qs("#cfle-events"),spotWrap=qs("#cfle-spotlight-section"),mainWrap=qs("#cfle-main-section");
  if(!mount)return{mount:null,spotWrap:null,mainWrap:null};
  if(!spotWrap){spotWrap=d.createElement("div");spotWrap.id="cfle-spotlight-section";spotWrap.className="cfle-section";mount.appendChild(spotWrap);}
  if(!mainWrap){mainWrap=d.createElement("div");mainWrap.id="cfle-main-section";mainWrap.className="cfle-section";mount.appendChild(mainWrap);}
  return{mount:mount,spotWrap:spotWrap,mainWrap:mainWrap};
}
function render(){
  ...
  if(spot.length){
    ...
    html+='</div>';
    containers.spotWrap.innerHTML=html;
  }else{
    containers.spotWrap.innerHTML="";
  }
  ...
  containers.mainWrap.innerHTML=html;
}
  ...
  fetch(CFG.feedUrl,{credentials:"same-origin",cache:"default"})
    ...
    .catch(function(){
      if(!state.events.length){
        ensureRenderContainers().mainWrap.innerHTML='<div class="cfle-empty"><strong>We couldn&rsquo;t load the programs.</strong><span>Please refresh the page and try again.</span></div>';
      }
    });
}
